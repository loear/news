const fetch           = require('node-fetch');
const AbortController = require('abort-controller');
const { url }         = require('./sina.js');
const Feed            = require('feed').Feed;
const fs              = require('fs/promises');
const process         = require('process');
const controller      = new AbortController();
const timeout         = setTimeout( () => { controller.abort(); }, 30000 ); // 30 秒后取消请求


async function main() {
  const feed      = new Feed({
    title       : '全球实时新闻资讯',
    description : '全球实时新闻资讯',
    link        : 'https://loear.github.io/news/',
    language    : 'zh-CN',
    generator   : 'sina news feed generator',
    feedLinks   : {
      json: 'https://loear.github.io/news/rss.json',
      rss: 'https://loear.github.io/news/rss.xml'
    },
  });
  const filterArr = [
    '比特币',
    '以太坊',
    '莱特币',
    '瑞波币',
    '疫苗',
    '疫情',
    '新冠',
    '央行',
    '联储',
    '中央银行',
    '财长',
    '财政部',
    '参议院',
    '众议院'
  ];
  const response = await fetch(url, {
    headers: {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.10130'},
    signal: controller.signal
  });
  if (response.status < 200 || response.status >= 300) {
    throw new Error('接口问题，错误的转态码: ' + response.status);
  }
  const json = await response.json();
  console.log(`请求数据成功`);
  const result = json.result || {};
  if (!result.status || result.status.code !== 0) return;
  const items = result.data.feed.list;
  console.log(`获取数据成功`);
  items.forEach(item => {
    if (!item.rich_text) return;
    // 文本过滤
    for (let i = 0; i < filterArr.length; i++) {
      if (item.rich_text.includes(filterArr[i])) {
        // console.log(item.rich_text);
        return;
      }
    }
    // 频道过滤
    if (item.tag && Array.isArray(item.tag) && item.tag.length) {
      const tags = item.tag;
      const tagFilterArr = ['5', '6', '7', '9'];  // 5市场6观点7央行9焦点
      for (let i = 0; i < tags.length; i++) {
        const tag = tags[i].id;
        if (tagFilterArr.includes(tag)) {
          // console.log(item.rich_text);
          return;
        }
      }
    }
    feed.addItem({
      title   : item.rich_text,
      id      : item.id,
      link    : item.docurl,
      content : '',
      date    : new Date(item.create_time + '+08:00'),
    });
  });
  console.log(`数据过滤完成`);
  await fs.rmdir('./dist', { recursive: true });
  console.log(`./dist 删除成功`);
  await fs.mkdir('./dist');
  console.log(`./dist 创建成功`);
  await fs.writeFile('./dist/rss.json', feed.json1());
  console.log(`rss.json 写入成功`);
  await fs.writeFile('./dist/rss.xml', feed.rss2());
  console.log(`rss.xml 写入成功`);
  await fs.copyFile('./template/index.html', `./dist/index.html`);
  await fs.copyFile('./template/page.js', `./dist/page.js`);
  console.log(`复制模板成功`);
}

main().catch(err => {
  console.log('错误：', err);
  process.exit(1);
}).finally(() => {
  clearTimeout(timeout);
});
